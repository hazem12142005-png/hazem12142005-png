# 👋 Hi, I'm Hazem Ahmed 0

I'm a Computer Science graduate from the Faculty of Computers and Artificial Intelligence. Passionate about building intelligent systems, solving real-world problems through code, and constantly learning new technologies.

## 💻 About Me
- 🎓 Studied Computer Science with a focus on AI and software engineering
- 🧠 Interested in Machine Learning, Web Development, and Data Science
- 🛠️ Love working with Python, JavaScript, and modern frameworks like React and Node.js
- 🌱 Currently exploring deep learning and cloud computing

## 🚀 Projects
- 🤖 **Smart Chatbot** – Built a conversational AI using NLP techniques
- 📈 **Data Insights Dashboard** – Interactive dashboard for visualizing complex datasets
- 🌐 **Portfolio Website** – Personal website showcasing my work and skills

## 📫 Let's Connect
- [LinkedIn](https://www.linkedin.com/)  
- [Email](mailto:hazem.ahmed@email.com)  
- [Portfolio](https://your-portfolio.com)

> “Code is the language of the future — and I'm here to speak it fluently.”

---

Thanks for visiting my profile! Feel free to check out my repositories and drop a ⭐ if you like what you see.
